...
// ── Shared question bank — SAME file used by web (data/Poshqquestionsdata.js)
//    and InspectionOfficerSurveyScreen.tsx. Never duplicate this data again;
//    always import it, so questionid <-> question text can never drift
//    between company app, officer app, and web admin. ──
import POSH_QUESTIONS from '../data/Poshqquestionsdata';
...
const TOTAL_PARTS = POSH_QUESTIONS.parts.length;
const TOTAL_QUESTIONS = POSH_QUESTIONS.parts.reduce((a: number, p: any) => a + p.questions.length, 0);
const ALL_QUESTIONS = POSH_QUESTIONS.parts.flatMap((p: any) => p.questions);
...
export default function PoshSurveyScreen({navigation}: any) {
  ...
  const allAnsweredInPart = useMemo(
    () => currentPart.questions.every((q: any) => answers[q.no]),
    [currentPart, answers],
  );
  ...
  const answeredCount = currentPart.questions.filter((q: any) => answers[q.no]).length;
  ...
  // ── POST /api/org/survey/submit — sends full answers (unchanged ones + edited ones) ──
  const handleSubmit = async () => {
    try {
      setLoading(true);
        ...
        <ScrollView contentContainerStyle={s.scroll}>
          ...
          <View style={s.card}>
            {POSH_QUESTIONS.parts.map((part: any, pIdx: number) => (
              <View key={pIdx} style={s.previewPartBlock}>
                <Text style={s.previewPartTitle}>{part.title?.[lang] || part.title?.en || `Part ${part.id}`}</Text>
  ...
  // ─────────────────────────────────────────────────────────
  if (submitted && canEdit) {
    const flaggedQuestions = ALL_QUESTIONS.filter((q: any) => editableIds.has(q.no));
    const allFlaggedAnswered = flaggedQuestions.every((q: any) => answers[q.no]);
  ...
  keyboardShouldPersistTaps="handled">
        ...
        <View style={s.card}>
          ...
          <View style={s.partHead}>
            ...
            <Text style={s.partTitle} numberOfLines={3}>
              {currentPart.title?.[lang] || currentPart.title?.en || `Part ${currentPart.id}`}
            </Text>
            <View style={s.partCount}>
              <Text style={s.partCountText}>{answeredCount}/{currentPart.questions.length}</Text>
            </View>
            ...
            <ScrollView style={s.modalBody} contentContainerStyle={{paddingBottom: 12}}>
              {POSH_QUESTIONS.parts.map((part: any, pIdx: number) => (
                <View key={pIdx} style={s.previewPartBlock}>
                  <View style={s.previewPartHead}>
                    <Text style={s.previewPartTitle} numberOfLines={2}>
                      {part.title?.[lang] || part.title?.en || `Part ${part.id}`}
                    </Text>
                    <TouchableOpacity
                      style={s.editBtn}
                      onPress={() => handleEditPart(pIdx)}
